...
func (m *PostgresDBRepo) AllItems(pageNo int, pageSize int) ([]*Item, error) {
	...
	query = `
		select
			id, rank, linkInfo, linkType, dispNm, mediumImgUrl, originPrice, salePrice, 
			discountPrice, discountType, discountText, discountRate, salesCount,
			shipText, reviewCount, reviewAvgPoint
		from
			wemakeprice.items
		where
		  batchid = $1
