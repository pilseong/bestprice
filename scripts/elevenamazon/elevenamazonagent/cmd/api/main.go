...
func connectToDB() *sql.DB {
	...
	for {
		...
		if err != nil {
			...
			dataSourceName = "postgresql://postgres:qwe123@192.168.50.167:5433/bestprice?sslmode=disable&timezone=UTC+9&connect_timeout=5"
			counts++
		} else {
...
func main() {
	...
	for {
		<-jt.t.C
		log.Println("Starting data fetching from 11st amazon")

		err := app.getData()
		if err != nil {
			log.Println("Data fetching has finished with error", err.Error())
		} else {
